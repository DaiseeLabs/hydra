...
sub _iterate {
    ...
    my $rjson = decode_json $content;
    my $pulls_list = $rjson->{values};
    foreach my $pull (@$pulls_list) {
        $pulls->{$pull->{id}} = $pull;
    }
    my $next = $rjson->{next};
    _iterate($next, $pulls, $ua) unless $next eq undef;
}
...
sub fetchInput {
    ...
    my $url = "https://api.bitbucket.org/2.0/repositories/$owner/$repo/pullrequests";
    my $token = getToken($bitbucket);
    $url = join "", $url, "?access_token=", $token;
    # Get pull request list
    my %pulls;
    my $ua = LWP::UserAgent->new();
    _iterate($url, \%pulls, $ua);
